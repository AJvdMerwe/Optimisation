...
def main():
    ...
    #     print(str(np.array(initial_s_0)) +" "+ str(np.sum(np.array(initial_s_0)*np.array(objective_function_weight))))
    #
    # quit()
    walk_lengths = []
    optima_solution = []
    ...
    for initial_s_0 in U:
        ...
        while local_search:
            ...
            # is selected
            best_z = calculate_weights(objective_function_weight, best_solution)  # best candidate solution fitness value is
            # calculated
            if best_z <= z_hat:
    ...
    logger.info("\noptima objective functions: " + str(np.array(optima_obj_func_value)))
    logger.info("walk lengths: " + str(np.array(walk_lengths)))
    logger.info("average walk lengths: " + str(np.array(walk_lengths).mean()))
    logger.info("proposed solutions: \n" + str(np.matrix(optima_solution)))
